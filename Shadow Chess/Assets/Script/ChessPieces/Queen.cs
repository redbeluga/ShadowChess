...
public class Queen : ChessPiece
{
  ...
  {
    bool isDiagonal = Math.Abs(loc.x - CurLoc.x) == Math.Abs(loc.y - CurLoc.y);
    bool isStraight = loc.x == CurLoc.x || loc.y == CurLoc.y;
    if (!isDiagonal && !isStraight) return false;
    int dX = Math.Sign(loc.x - CurLoc.x);
    int dY = Math.Sign(loc.y - CurLoc.y);
